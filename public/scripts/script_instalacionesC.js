$(function () {
   ...
   function init() {
      ...
      $('#form-instalacion').on('submit', function () {
   ...
   function events() {
      ...
      $('#est_instalacion option:eq(2)').prop('disabled', true);
   ...
   function ubigeo() {
      ...
      $('#region').on('change', function () {
         ...
         $.ajax({
            ...
            beforeSend: function () {
               $('#provincia').select2({
                  placeholder: 'CARGANDO PROVINCIAS...'
               });
            },
            success: function (provincia) {
               var old = $('#provincia').data('old') != '' ? $('#provincia').data('old') : '';
               $('#provincia').empty();
               $('#comuna').empty();
               $('#provincia').select2({
                  width: '100%',
      ...
      $('#provincia').on('change', function () {
         ...
         $.ajax({
            url: route('instalaciones_comunas.data') + '?provinciaID=' + comuna_id,
            type: "GET",
            dataType: "json",
            beforeSend: function () {
               $('#comuna').select2({
            ...
            success: function (comuna) {
               var old = $('#comuna').data('old') != '' ? $('#comuna').data('old') : '';
               $('#comuna').empty();
               $('#comuna').select2({
                  width: '100%',
                  placeholder: "SELECCIONAR...",
   ...
   function validations() {
      ...
      const mensajes = {
         ...
         'nom_fantasia': {
            maxlength: 'El nombre de fantasia del proveedor debe contener como máximo 50 caracteres.',
         },
         'nrodoc_instalacion': {
